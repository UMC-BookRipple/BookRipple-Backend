...
import com.bookripple.api.domain.library.code.LibraryErrorCode;
...
public class LibraryQueryServiceImpl implements LibraryQueryService {
  ...
  @Override
  public LibraryItemListRes getMyLibrary(Long memberId, LibraryStatus status, Long lastId,
      int size) {
    ...
    if (status == LibraryStatus.LIKED) {
      ...
      List<ReadingProgress> fetched = (lastId == null)
          ...
          : readingProgressRepository.findByMemberIdAndIsLikedTrueAndIdLessThanOrderByIdDesc(
              memberId, lastId, pageable);
  ...
  @Override
  public LibraryDto.DeleteRes deleteBooks(Long memberId, LibraryStatus status,
      LibraryDto.DeleteReq request) {
  ...
  public LibraryBookDetailRes getMyLibraryBookDetail(Long memberId, Long bookId) {
    ...
    if (item.getStatus() == LibraryStatus.READING) {
      // 첫 독서 기록 조회
      var firstRecordOpt = readingRecordRepository.findFirstByMemberIdAndBookIdOrderByCreatedAtAsc(
          memberId, bookId);
      // 최근 독서 기록 조회
    ...
    return LibraryBookDetailRes.ofWithReadingStats(item, progress, readingSpeed,
  ...
  public LibraryBookSummaryListRes getMyLibraryBooksSummary(Long memberId) {
    ...
    for (LibraryItem item : items) {
      ...
      // 읽기 진행 상황 조회
      ReadingProgress progress = readingProgressRepository.findByMemberIdAndBookId(memberId,
          bookId);
      ...
      if (item.getStatus() == LibraryStatus.READING) {
        ...
        var firstRecordOpt = readingRecordRepository.findFirstByMemberIdAndBookIdOrderByCreatedAtAsc(
            memberId, bookId);
        // 최근 독서 기록 조회
        var latestRecordOpt = readingRecordRepository.findLatestByMemberIdAndBookId(memberId,
            bookId);
      ...
      summaries.add(
          LibraryBookSummaryRes.ofWithReadingStats(item, progress, estimatedDaysToCompletion));
    }
